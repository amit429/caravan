...
import { CheckCircle2, CircleDashed, ChevronRight, BookOpen } from "lucide-react";
...
export default async function YouPage({ params }: { params: Promise<{ tripId: string }> }) {
  ...
  const [{ count: factCount }, { data: yourTasks }, { count: ideaCount }] = await Promise.all([
    ...
    supabase.from("ideas").select("*", { count: "exact", head: true }).eq("trip_id", tripId).eq("member_id", caller.id),
  ]);
  const intakeDone = (factCount ?? 0) > 0;
  const pendingTasks = ((yourTasks ?? []) as TaskRow[]).filter((t) => !t.done).length;
  const scrapbookCount = (factCount ?? 0) + (ideaCount ?? 0);
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="flex flex-col gap-2 px-4 pt-3">
        ...
        <Link
          href={`/trip/${tripId}/you/scrapbook`}
          className="flex items-center gap-2.5 rounded-lg bg-card px-3.5 py-2.5 transition-colors hover:bg-sunk"
        >
          <BookOpen className="size-4 shrink-0 text-ink-3" />
          <span className="flex-1 text-sm font-medium">
            Your scrapbook{scrapbookCount > 0 ? ` — ${scrapbookCount}` : ""}
          </span>
          <ChevronRight className="size-4 shrink-0 text-ink-3" />
        </Link>
      </div>
